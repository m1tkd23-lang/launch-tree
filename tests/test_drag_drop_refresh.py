...
def test_tree_drop_uses_model_reset_refresh(window, monkeypatch):
    source_index = window._source_index_for_node_id("x")
    target_index = window._source_index_for_node_id("b")
    source_proxy = window.proxy_model.mapFromSource(source_index)
    target_proxy = window.proxy_model.mapFromSource(target_index)
    ...
    called = {"count": 0}
    original = window.source_model.force_reset_refresh

    def wrapped():
        called["count"] += 1
        return original()

    monkeypatch.setattr(window.source_model, "force_reset_refresh", wrapped)
